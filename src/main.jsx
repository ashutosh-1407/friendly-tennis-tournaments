...
function ScoreEntryModal({ match, onClose, onSave, isSaving, error }) {
  ...
  return <div className="score-backdrop" role="presentation" onMouseDown={onClose}><section className="score-modal" role="dialog" aria-modal="true" aria-labelledby="score-title" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={onClose} aria-label="Close">×</button><p className="eyebrow">ENTER RESULT</p><h2 id="score-title">{match.player_one_name} <span>vs</span> {match.player_two_name}</h2><p className="score-help">Best of three sets. If tied at one set each, enter a first-to-10, win-by-two match tiebreak.</p><form onSubmit={(event) => { event.preventDefault(); onSave(sets) }}><div className="set-head"><span>Set</span><span>{match.player_one_name}</span><span>{match.player_two_name}</span></div>{sets.slice(0, 2).map((set, index) => <div className="score-set" key={index}><span>Set {index + 1}</span><input aria-label={`${match.player_one_name} games in set ${index + 1}`} inputMode="numeric" value={set.playerOneGames} onChange={(event) => updateSet(index, 'playerOneGames', event.target.value)} /><input aria-label={`${match.player_two_name} games in set ${index + 1}`} inputMode="numeric" value={set.playerTwoGames} onChange={(event) => updateSet(index, 'playerTwoGames', event.target.value)} />{needsTiebreak(set) && <div className="set-tiebreak"><span>7-point tiebreak</span><input aria-label={`${match.player_one_name} tiebreak points in set ${index + 1}`} inputMode="numeric" value={set.tiebreakPlayerOne} onChange={(event) => updateSet(index, 'tiebreakPlayerOne', event.target.value)} /><input aria-label={`${match.player_two_name} tiebreak points in set ${index + 1}`} inputMode="numeric" value={set.tiebreakPlayerTwo} onChange={(event) => updateSet(index, 'tiebreakPlayerTwo', event.target.value)} /></div>}</div>)}<div className="score-set score-set--decider"><span>Match tiebreak <small>to 10</small></span><input aria-label={`${match.player_one_name} deciding tiebreak points`} inputMode="numeric" value={sets[2].playerOnePoints} onChange={(event) => updateSet(2, 'playerOnePoints', event.target.value)} /><input aria-label={`${match.player_two_name} deciding tiebreak points`} inputMode="numeric" value={sets[2].playerTwoPoints} onChange={(event) => updateSet(2, 'playerTwoPoints', event.target.value)} /></div>{error && <p className="score-error" role="alert">{error}</p>}<div className="score-actions"><button type="button" className="cancel-button" onClick={onClose}>Cancel</button><button type="submit" className="primary-action" disabled={isSaving}>{isSaving ? 'Saving…' : 'Save result'}</button></div></form></section></div>
}
...
function App() {
  ...
  const [tournamentDraft, setTournamentDraft] = React.useState({ name: '', startDate: '', endDate: '', location: '', maxPlayers: '', description: '' })
  ...
  function updateTournamentDraft(event) {
    ...
    if (event.target.name === 'startDate' && (!tournamentDraft.endDate || tournamentDraft.endDate < event.target.value)) nextDraft.endDate = event.target.value
  ...
  async function createTournament(event) {
    ...
    try {
      ...
      setTournamentDraft({ name: '', startDate: '', endDate: '', location: '', maxPlayers: '', description: '' })
      setActiveTab('tournaments')
  ...
  return (
    <main className="app-shell">
      ...
      {activeTab === 'detail' ? (
        ...
        <section className="page create-page" aria-labelledby="create-title">
          ...
          <form className="tournament-form" onSubmit={createTournament}>
            ...
            <div className="form-grid"><label>Start date<input type="date" name="startDate" value={tournamentDraft.startDate} onChange={updateTournamentDraft} required /></label><label>End date<input type="date" name="endDate" value={tournamentDraft.endDate} onChange={updateTournamentDraft} min={tournamentDraft.startDate || undefined} required /></label></div>
            <label>Player limit <span className="optional">(optional, maximum 32)</span><input type="number" name="maxPlayers" value={tournamentDraft.maxPlayers} onChange={updateTournamentDraft} min="2" max="32" placeholder="16" /></label>
        ...
        <section className="page leaderboard" aria-labelledby="leaderboard-title">
          ...
          <div className="rating-note"><span>✦</span><div><strong>500 points for a tournament win.</strong><p>Later-round wins are worth more; points update as scores are recorded.</p></div></div>
      ...
      {scoreMatch && <ScoreEntryModal match={scoreMatch} onClose={() => setScoreMatch(null)} onSave={saveScore} isSaving={isSavingScore} error={scoreError} />}
    </main>
  )
}
