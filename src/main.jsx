...
function TournamentDraw({ draw, isOrganizer, tournamentStatus, drawActionPending, onManage, userId, onEnterScore, onAwardBye }) {
  ...
  return <div className="draw-panel">
    ...
    {!matches.length ? <div className="draw-locked"><span>◌</span><h2>Bracket not generated yet</h2><p>The organizer will create the bracket once registrations are in.</p></div> : <div className="bracket-scroller"><div className="bracket-board" style={{ '--round-count': draw.rounds.length }}>{draw.rounds.map((round, roundIndex) => {
      ...
      const matchCount = Math.max(1, roundMatches.length || Math.ceil(openingMatches.length / (2 ** roundIndex)))
      ...
      return <section className={`bracket-column bracket-column--${roundIndex}`} key={round}><h3>{roundLabel(round)}</h3><div className="bracket-matches">{Array.from({ length: matchCount }, (_, matchIndex) => {
        ...
        const winnerPlaceholder = (priorMatch, fallback) => {
          if (!priorMatch?.winner_name) return fallback
          const seed = priorMatch.winner_user_id === priorMatch.player_one_id ? priorMatch.player_one_seed : priorMatch.player_two_seed
          return <>{priorMatch.winner_name}{seed && <sup className="draw-seed-number">[{seed}]</sup>}</>
        }
        const firstPlaceholder = roundIndex === 0 ? null : winnerPlaceholder(priorRoundMatches[matchIndex * 2], `Winner of ${roundLabel(draw.rounds[roundIndex - 1])} ${matchIndex * 2 + 1}`)
        const secondPlaceholder = roundIndex === 0 ? null : winnerPlaceholder(priorRoundMatches[matchIndex * 2 + 1], `Winner of ${roundLabel(draw.rounds[roundIndex - 1])} ${matchIndex * 2 + 2}`)
        const playerOneLabel = match?.player_one_name ? <>{match.player_one_display_name || match.player_one_name}{match.player_one_seed && <sup className="draw-seed-number">[{match.player_one_seed}]</sup>}</> : (roundIndex === 0 && match?.player_two_name ? 'BYE' : firstPlaceholder)
        const playerTwoLabel = match?.player_two_name ? <>{match.player_two_display_name || match.player_two_name}{match.player_two_seed && <sup className="draw-seed-number">[{match.player_two_seed}]</sup>}</> : (roundIndex === 0 && match?.player_one_name ? 'BYE' : secondPlaceholder)
        const canEnterScore = tournamentStatus === 'current' && Boolean(match?.id) && (match.status !== 'completed' || isOrganizer) && (isOrganizer || match.player_one_id === userId || match.player_two_id === userId)
        const canShowWinner = ['current', 'past'].includes(tournamentStatus) && Boolean(match?.winner_user_id)
        return <div className={`bracket-match ${match ? '' : 'bracket-match--pending'} ${canEnterScore ? 'bracket-match--editable' : ''}`} key={match?.id || match?.node_id || matchIndex} role={canEnterScore ? 'button' : undefined} tabIndex={canEnterScore ? 0 : undefined} onClick={() => canEnterScore && onEnterScore(match)} onKeyDown={(event) => canEnterScore && event.key === 'Enter' && onEnterScore(match)} title={canEnterScore ? 'Enter match score' : undefined}><div className={`bracket-player ${canShowWinner && match.winner_user_id === match.player_one_id ? 'bracket-player--winner' : ''}`}><span>{playerOneLabel}</span><b>{match?.player_one_score || '—'}</b></div><div className={`bracket-player ${canShowWinner && match.winner_user_id === match.player_two_id ? 'bracket-player--winner' : ''}`}><span>{playerTwoLabel}</span><b>{match?.player_two_score || '—'}</b></div>{isOrganizer && tournamentStatus === 'current' && match?.id && match.status !== 'completed' && <div className="bye-controls"><button onClick={(event) => { event.stopPropagation(); onAwardBye(match, match.player_one_id) }}>Bye → {match.player_one_display_name || match.player_one_name}</button><button onClick={(event) => { event.stopPropagation(); onAwardBye(match, match.player_two_id) }}>Bye → {match.player_two_display_name || match.player_two_name}</button></div>}</div>
      })}</div></section>
    })}</div></div>}
  </div>
}
...
function ScoreEntryModal({ match, onClose, onSave, isSaving, error, isSingleSet }) {
  ...
  const playerOneName = match.player_one_display_name || match.player_one_name
  const playerTwoName = match.player_two_display_name || match.player_two_name
  return <div className="score-backdrop" role="presentation" onMouseDown={onClose}><section className="score-modal" role="dialog" aria-modal="true" aria-labelledby="score-title" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={onClose} aria-label="Close">×</button><p className="eyebrow">ENTER RESULT</p><h2 id="score-title">{playerOneName} <span>vs</span> {playerTwoName}</h2><p className="score-help">{isSingleSet ? 'One standard set. At 6–6, enter the 7-point, win-by-two tiebreak.' : 'Best of three sets. If tied at one set each, enter a first-to-10, win-by-two match tiebreak.'}</p><form onSubmit={(event) => { event.preventDefault(); onSave(sets) }}><div className="set-head"><span>Set</span><span>{playerOneName}</span><span>{playerTwoName}</span></div>{sets.slice(0, isSingleSet ? 1 : 2).map((set, index) => <div className="score-set" key={index}><span>Set {index + 1}</span><input aria-label={`${playerOneName} games in set ${index + 1}`} inputMode="numeric" value={set.playerOneGames} onChange={(event) => updateSet(index, 'playerOneGames', event.target.value)} /><input aria-label={`${playerTwoName} games in set ${index + 1}`} inputMode="numeric" value={set.playerTwoGames} onChange={(event) => updateSet(index, 'playerTwoGames', event.target.value)} />{needsTiebreak(set) && <div className="set-tiebreak"><span>7-point tiebreak</span><input aria-label={`${playerOneName} tiebreak points in set ${index + 1}`} inputMode="numeric" value={set.tiebreakPlayerOne} onChange={(event) => updateSet(index, 'tiebreakPlayerOne', event.target.value)} /><input aria-label={`${playerTwoName} tiebreak points in set ${index + 1}`} inputMode="numeric" value={set.tiebreakPlayerTwo} onChange={(event) => updateSet(index, 'tiebreakPlayerTwo', event.target.value)} /></div>}</div>)}{!isSingleSet && <div className="score-set score-set--decider"><span>Match tiebreak <small>to 10</small></span><input aria-label={`${playerOneName} deciding tiebreak points`} inputMode="numeric" value={sets[2].playerOnePoints} onChange={(event) => updateSet(2, 'playerOnePoints', event.target.value)} /><input aria-label={`${playerTwoName} deciding tiebreak points`} inputMode="numeric" value={sets[2].playerTwoPoints} onChange={(event) => updateSet(2, 'playerTwoPoints', event.target.value)} /></div>}{error && <p className="score-error" role="alert">{error}</p>}<div className="score-actions"><button type="button" className="cancel-button" onClick={onClose}>Cancel</button><button type="submit" className="primary-action" disabled={isSaving}>{isSaving ? 'Saving…' : 'Save result'}</button></div></form></section></div>
}
